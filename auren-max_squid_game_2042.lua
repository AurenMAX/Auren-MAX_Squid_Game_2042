-- =============================================
-- Auren MAX - Squid Game 2042 v10.0
-- Green-Black Luxury | ESP + Fly
...
local Config = {
    ...
    Aimlock         = false,
    AimlockRadius   = 100,
    AimlockColor    = Color3.fromRGB(0, 255, 120),
    Tracers         = false,
    ShowHealth      = false,
    ShowDistance     = false,
...
local Lang = {
    EN = {
        ...
        -- Combat tab
        AimlockSec = "AIMLOCK",
        EnableAimlock = "Enable Aimlock",
        AimlockRadius = "Lock Radius (px)",
        AimlockTip = "Locks aim to nearest enemy head in circle.",
        TracersSec = "ESP TRACERS",
        ...
        InfAmmoCustom = "Type gun name...",
        InfAmmoBugBtn = "INFINITE AMMO",
        InfAmmoReady = "READY",
        InfAmmoWorking = "WORKING",
        InfAmmoSuccess = "SUCCESS!",
        InfAmmoNotFound = "NOT FOUND!",
        -- Color tab
        ...
        AimlockColors = "AIMLOCK CIRCLE",
        AimlockCircleColor = "Circle Color",
        -- Settings tab
        UIScale = "UI SCALE",
    ...
    TH = {
        ...
        AimlockSec = "ล็อกเป้า",
        EnableAimlock = "เปิดล็อกเป้า",
        AimlockRadius = "รัศมีล็อก (px)",
        AimlockTip = "ล็อกเป้าไปที่หัวศัตรูที่อยู่ในวง",
        TracersSec = "เส้น ESP",
        ...
        InfAmmoCustom = "พิมพ์ชื่อปืน...",
        InfAmmoBugBtn = "กระสุนไม่จำกัด",
        InfAmmoReady = "พร้อม",
        InfAmmoWorking = "กำลังทำงาน",
        InfAmmoSuccess = "สำเร็จ!",
        InfAmmoNotFound = "ไม่พบ!",
        HighlightColors = "สีไฮไลท์",
        FillColor = "สีเติม",
        OutlineColor = "สีขอบ",
        FillOpacity = "ความทึบสีเติม",
        OutlineOpacity = "ความทึบสีขอบ",
        AimlockColors = "วงกลมล็อกเป้า",
        AimlockCircleColor = "สีวงกลม",
        UIScale = "ขนาด UI",
...
IS.AnchorPoint = Vector2.new(0.5,0.5); IS.BackgroundTransparency = 1; IS.Text = "Squid Game 2042"
...
SL.BackgroundTransparency = 1; SL.Text = "Squid Game 2042"; SL.TextColor3 = T.TxD; SL.TextSize = 9
...
Tog(v1, "Always On Top", true, 3, function(v) Config.DepthMode = v; Rebuild() end, "AlwaysOnTop")
...
Tog(al1, "Enable Aimlock", false, 1, function(v)
    Config.Aimlock = v
    if aimlockCircle then aimlockCircle.Visible = v end
end, "EnableAimlock")
Sld(al1, "Lock Radius (px)", 30, 300, 100, 2, function(v)
    Config.AimlockRadius = v
    if aimlockCircle then
        aimlockCircle.Size = UDim2.new(0, v * 2, 0, v * 2)
    end
end, "AimlockRadius")

local alInfo = Instance.new("Frame"); alInfo.Size = UDim2.new(1,0,0,30); alInfo.BackgroundTransparency = 1; alInfo.LayoutOrder = 3; alInfo.Parent = al1
local alLbl = Instance.new("TextLabel"); alLbl.Size = UDim2.new(1,-24,1,0); alLbl.Position = UDim2.new(0,12,0,0)
alLbl.BackgroundTransparency = 1; alLbl.Text = L("AimlockTip")
alLbl.TextColor3 = T.TxD; alLbl.TextSize = 8; alLbl.Font = Enum.Font.Gotham; alLbl.TextXAlignment = Enum.TextXAlignment.Left
...
local trLbl = Instance.new("TextLabel"); trLbl.Size = UDim2.new(1,-24,1,0); trLbl.Position = UDim2.new(0,12,0,0)
trLbl.BackgroundTransparency = 1; trLbl.Text = L("TracersTip")
trLbl.TextColor3 = T.TxD; trLbl.TextSize = 8; trLbl.Font = Enum.Font.Gotham; trLbl.TextXAlignment = Enum.TextXAlignment.Left
...
local iaLbl = Instance.new("TextLabel"); iaLbl.Size = UDim2.new(1,-24,1,0); iaLbl.Position = UDim2.new(0,12,0,0)
iaLbl.BackgroundTransparency = 1; iaLbl.Text = L("InfAmmoTip")
iaLbl.TextColor3 = T.TxD; iaLbl.TextSize = 8; iaLbl.Font = Enum.Font.Gotham; iaLbl.TextXAlignment = Enum.TextXAlignment.Left
iaLbl.TextWrapped = true; iaLbl.Parent = iaInfo
table.insert(TranslatableUI, {obj=iaLbl, key="InfAmmoTip"})

-- Weapon lists
local infAmmoWeapons = {
...
-- Gun list container (shown/hidden when category is clicked)
local iaGunList = Instance.new("Frame"); iaGunList.Size = UDim2.new(1,0,0,0); iaGunList.AutomaticSize = Enum.AutomaticSize.Y
iaGunList.BackgroundTransparency = 1; iaGunList.LayoutOrder = 5; iaGunList.Visible = false; iaGunList.Parent = ia1
local iaGunListLayout = Instance.new("UIListLayout"); iaGunListLayout.SortOrder = Enum.SortOrder.LayoutOrder
...
local function makeGunBtn(gunName, order)
    local gb = Instance.new("TextButton"); gb.Size = UDim2.new(1,-24,0,26)
    gb.Position = UDim2.new(0,12,0,0); gb.BackgroundColor3 = T.SfL; gb.BorderSizePixel = 0
    gb.Text = gunName; gb.TextColor3 = T.Tx; gb.TextSize = 10; gb.Font = Enum.Font.Gotham
    gb.AutoButtonColor = false; gb.LayoutOrder = order; gb.Parent = iaGunList; Crn(gb,5)
    gb.MouseEnter:Connect(function() Tw(gb,{BackgroundColor3=T.SfH},0.12) end)
    gb.MouseLeave:Connect(function() Tw(gb,{BackgroundColor3=T.SfL},0.12) end)
    gb.MouseButton1Click:Connect(function()
        if runAmmoBugByName then task.spawn(function() runAmmoBugByName(gunName) end) end
    end)
    return gb
end

-- Current open category
local iaCurrentCat = nil

-- Helper: open a weapon category
local function openAmmoCat(catName)
    -- Clear existing gun buttons
    for _, v in pairs(iaGunList:GetChildren()) do
        if v:IsA("TextButton") then v:Destroy() end
    end
    if iaCurrentCat == catName then
        iaGunList.Visible = false; iaCurrentCat = nil; return
    end
    iaCurrentCat = catName
    local guns = infAmmoWeapons[catName]
    if not guns then return end
    for i, gunName in ipairs(guns) do
        makeGunBtn(gunName, i)
    end
    iaGunList.Visible = true
end

-- Category buttons row
local iaCatRow = Instance.new("Frame"); iaCatRow.Size = UDim2.new(1,0,0,28); iaCatRow.BackgroundTransparency = 1
iaCatRow.LayoutOrder = 2; iaCatRow.Parent = ia1
local iaCatLay = Instance.new("UIListLayout"); iaCatLay.FillDirection = Enum.FillDirection.Horizontal
iaCatLay.SortOrder = Enum.SortOrder.LayoutOrder; iaCatLay.Padding = UDim.new(0,4); iaCatLay.Parent = iaCatRow
local iaCatPad = Instance.new("UIPadding"); iaCatPad.PaddingLeft = UDim.new(0,8); iaCatPad.PaddingRight = UDim.new(0,8); iaCatPad.Parent = iaCatRow

local function makeCatBtn(text, catKey, color, order, langKey)
    local cb = Instance.new("TextButton"); cb.Size = UDim2.new(0.32,0,0,24)
    cb.BackgroundColor3 = color; cb.BorderSizePixel = 0; cb.Text = text
    cb.TextColor3 = Color3.new(1,1,1); cb.TextSize = 9; cb.Font = Enum.Font.GothamBold
    cb.AutoButtonColor = false; cb.LayoutOrder = order; cb.Parent = iaCatRow; Crn(cb,5)
    if langKey then table.insert(TranslatableUI, {obj=cb, key=langKey}) end
    cb.MouseEnter:Connect(function() Tw(cb,{BackgroundColor3=Color3.new(math.min(color.R*1.3,1),math.min(color.G*1.3,1),math.min(color.B*1.3,1))},0.12) end)
    cb.MouseLeave:Connect(function() Tw(cb,{BackgroundColor3=color},0.12) end)
    cb.MouseButton1Click:Connect(function() openAmmoCat(catKey) end)
    return cb
end

makeCatBtn(L("InfAmmoSniper"), "Sniper", Color3.fromRGB(0,80,150), 1, "InfAmmoSniper")
makeCatBtn(L("InfAmmoRifle"), "Rifle", Color3.fromRGB(150,80,0), 2, "InfAmmoRifle")
makeCatBtn(L("InfAmmoSpecial"), "Special", Color3.fromRGB(100,0,150), 3, "InfAmmoSpecial")

-- Custom gun name input
local iaInputRow = Instance.new("Frame"); iaInputRow.Size = UDim2.new(1,0,0,34); iaInputRow.BackgroundTransparency = 1
iaInputRow.LayoutOrder = 3; iaInputRow.Parent = ia1
local iaInputPad = Instance.new("UIPadding"); iaInputPad.PaddingLeft = UDim.new(0,8); iaInputPad.PaddingRight = UDim.new(0,8); iaInputPad.Parent = iaInputRow

local iaInputBox = Instance.new("Frame"); iaInputBox.Size = UDim2.new(0.62,0,0,28); iaInputBox.Position = UDim2.new(0,0,0,3)
iaInputBox.BackgroundColor3 = T.SfL; iaInputBox.BorderSizePixel = 0; iaInputBox.Parent = iaInputRow; Crn(iaInputBox,6); Stk(iaInputBox,T.Bd,1,0.3)
local iaInput = Instance.new("TextBox"); iaInput.Size = UDim2.new(1,-12,1,0); iaInput.Position = UDim2.new(0,6,0,0)
iaInput.BackgroundTransparency = 1; iaInput.Text = ""; iaInput.PlaceholderText = L("InfAmmoCustom")
iaInput.PlaceholderColor3 = T.TxD; iaInput.TextColor3 = T.Tx; iaInput.TextSize = 10
iaInput.Font = Enum.Font.Gotham; iaInput.ClearTextOnFocus = false; iaInput.Parent = iaInputBox
table.insert(TranslatableUI, {obj=iaInput, key="InfAmmoCustom", prop="PlaceholderText"})

local iaBugBtn = Instance.new("TextButton"); iaBugBtn.Size = UDim2.new(0.35,0,0,28); iaBugBtn.Position = UDim2.new(0.65,0,0,3)
iaBugBtn.BackgroundColor3 = T.Ac; iaBugBtn.BorderSizePixel = 0; iaBugBtn.Text = L("InfAmmoBugBtn")
iaBugBtn.TextColor3 = Color3.fromRGB(0,0,0); iaBugBtn.TextSize = 10; iaBugBtn.Font = Enum.Font.GothamBold
iaBugBtn.AutoButtonColor = false; iaBugBtn.Parent = iaInputRow; Crn(iaBugBtn,6)
table.insert(TranslatableUI, {obj=iaBugBtn, key="InfAmmoBugBtn"})
iaBugBtn.MouseEnter:Connect(function() Tw(iaBugBtn,{BackgroundColor3=T.AcG},0.12) end)
iaBugBtn.MouseLeave:Connect(function() Tw(iaBugBtn,{BackgroundColor3=T.Ac},0.12) end)
iaBugBtn.MouseButton1Click:Connect(function()
    if runAmmoBugByName and iaInput.Text ~= "" then
        task.spawn(function() runAmmoBugByName(iaInput.Text) end)
    end
end)

-- Status label
local iaStatusRow = Instance.new("Frame"); iaStatusRow.Size = UDim2.new(1,0,0,22); iaStatusRow.BackgroundTransparency = 1
iaStatusRow.LayoutOrder = 4; iaStatusRow.Parent = ia1
infAmmoStatusLbl = Instance.new("TextLabel"); infAmmoStatusLbl.Size = UDim2.new(1,-24,1,0); infAmmoStatusLbl.Position = UDim2.new(0,12,0,0)
infAmmoStatusLbl.BackgroundTransparency = 1; infAmmoStatusLbl.Text = L("InfAmmoReady")
infAmmoStatusLbl.TextColor3 = T.TxS; infAmmoStatusLbl.TextSize = 9; infAmmoStatusLbl.Font = Enum.Font.GothamBold
infAmmoStatusLbl.TextXAlignment = Enum.TextXAlignment.Left; infAmmoStatusLbl.Parent = iaStatusRow
...
local co2 = Sec(coP, "AIMLOCK CIRCLE", Ic.Target, 2, "AimlockColors")
CPick(co2, "Circle Color", Config.AimlockColor, 1, function(c)
    Config.AimlockColor = c
    if aimlockStroke then aimlockStroke.Color = c end
    if aimlockDot then aimlockDot.BackgroundColor3 = c end
...
function Build(plr)
    ...
    -- Skip teammates when EnemyOnly is enabled
    if Config.EnemyOnly and LocalPlayer.Team and plr.Team == LocalPlayer.Team then return end
...
local renderConn = RunService.RenderStepped:Connect(function()
    ...
    -- Raycast params for line-of-sight check
    local losParams = RaycastParams.new()
    losParams.FilterType = Enum.RaycastFilterType.Exclude

    -- Collect all player characters to exclude from LOS raycasts
    -- (prevents dead ragdolls from blocking LOS to other enemies)
    local allChars = {myC or workspace}
    for _, plr in ipairs(Players:GetPlayers()) do
        if plr.Character then table.insert(allChars, plr.Character) end
    end
    ...
    for plr, d in pairs(ESP) do
        ...
        -- Line-of-sight check: gray if behind SOLID walls or dead, normal if visible
        -- Skips transparent parts, non-collidable parts (grass, foliage, etc.)
        if d.hl and d.hl.Parent then
            local head = ch:FindFirstChild("Head")
            local hum = ch:FindFirstChildOfClass("Humanoid")
            local camPos = Camera.CFrame.Position

            -- Dead or no head = force gray
            if not head or not hum or hum.Health <= 0 then
                d.hl.FillColor = Color3.fromRGB(180, 180, 180)
                d.hl.OutlineColor = Color3.fromRGB(220, 220, 220)
            else
                local dir = head.Position - camPos
                local blocked = false
                local origin = camPos
                local remaining = dir
                -- Exclude all characters; only test against world geometry
                local ignore = {}
                for _, c in ipairs(allChars) do table.insert(ignore, c) end

                -- Multi-raycast: pass through non-solid/transparent objects
                for _ = 1, 10 do
                    losParams.FilterDescendantsInstances = ignore
                    local result = workspace:Raycast(origin, remaining, losParams)
                    if not result then
                        break -- clear line of sight
                    end
                    local part = result.Instance
                    -- Solid wall = CanCollide true + opaque enough
                    if part.CanCollide and part.Transparency < 0.3 then
                        blocked = true
                        break
                    end
                    -- Non-solid hit (grass, transparent): skip it and continue
                    table.insert(ignore, part)
                    origin = result.Position + remaining.Unit * 0.1
                    remaining = head.Position - origin
                    if remaining.Magnitude < 0.5 then break end
                end
                ...
                if blocked then
                    ...
                    d.hl.FillColor = Config.FillColor
                    d.hl.OutlineColor = Config.OutlineColor
                end
            end
        end
...
aimlockCircle.AnchorPoint = Vector2.new(0.5, 0.5)
aimlockCircle.Size = UDim2.new(0, Config.AimlockRadius * 2, 0, Config.AimlockRadius * 2)
aimlockCircle.Position = UDim2.new(0.5, 0, 0.5, 0)
aimlockCircle.BackgroundTransparency = 1
aimlockCircle.Visible = false
aimlockCircle.ZIndex = 50
aimlockCircle.Parent = Gui

aimlockStroke = Instance.new("UIStroke")
aimlockStroke.Color = Config.AimlockColor
aimlockStroke.Thickness = 1.5
aimlockStroke.Transparency = 0.3
aimlockStroke.Parent = aimlockCircle
Crn(aimlockCircle, 9999) -- fully round

-- Small dot at center
aimlockDot = Instance.new("Frame")
aimlockDot.AnchorPoint = Vector2.new(0.5, 0.5)
aimlockDot.Size = UDim2.new(0, 4, 0, 4)
aimlockDot.Position = UDim2.new(0.5, 0, 0.5, 0)
aimlockDot.BackgroundColor3 = Config.AimlockColor
aimlockDot.BorderSizePixel = 0
aimlockDot.ZIndex = 51
aimlockDot.Parent = Gui
Crn(aimlockDot, 9999)
...
RunService:BindToRenderStep(AIMLOCK_BIND_NAME, 201, function()
    ...
    -- Validate current target still alive and in game
    if aimlockTarget then
        local valid = false
        pcall(function()
            if aimlockTarget.Parent and aimlockTarget.Character then
                ...
                if head and hum and hum.Health > 0 then
                    ...
                    if onScr then valid = true end
                end
            end
        end)
        if not valid then aimlockTarget = nil end
    end

    -- Acquire new target if none: pick nearest enemy by 3D distance within screen radius
    if not aimlockTarget then
        local myC = LocalPlayer.Character
        local myH = myC and myC:FindFirstChild("HumanoidRootPart")
        if myH then
            local bestPlr, bestDist3D = nil, math.huge
            for _, plr in ipairs(Players:GetPlayers()) do
                ...
                local dist3D = (myH.Position - head.Position).Magnitude
                if dist3D < bestDist3D then
                    bestDist3D = dist3D
                    bestPlr = plr
                end
            end
            aimlockTarget = bestPlr
        end
    end

    -- Aim at target head: only change camera ROTATION, keep position from game
...
runAmmoBugByName = function(gunName)
    if DESTROYED then return end
    if not gunName or gunName == "" then return end
    local ch = LocalPlayer.Character
    local bp = LocalPlayer:FindFirstChildOfClass("Backpack")

    -- Find gun by name in Character or Backpack
    local gun = (ch and ch:FindFirstChild(gunName)) or (bp and bp:FindFirstChild(gunName))
    if gun then
        -- Update status: WORKING
        if infAmmoStatusLbl then
            infAmmoStatusLbl.Text = L("InfAmmoWorking") .. ": " .. gunName:upper()
            infAmmoStatusLbl.TextColor3 = Color3.fromRGB(255, 220, 40)
        end

        -- Clone bug
        pcall(function()
            local storage = Instance.new("Folder", LocalPlayer)
            storage.Name = "BugTemp"
            for _, child in pairs(gun:GetChildren()) do
                child:Clone().Parent = storage
            end
            task.wait(0.05)
        ...
        if infAmmoStatusLbl then
            infAmmoStatusLbl.Text = L("InfAmmoSuccess")
            infAmmoStatusLbl.TextColor3 = T.Ac
        end
    else
        -- Update status: NOT FOUND
        if infAmmoStatusLbl then
            infAmmoStatusLbl.Text = L("InfAmmoNotFound") .. " " .. gunName
            infAmmoStatusLbl.TextColor3 = T.Rd
        end
    end
    ...
    task.wait(1.5)
    if infAmmoStatusLbl then
        infAmmoStatusLbl.Text = L("InfAmmoReady")
...
local function cleanTracer(plr)
    if tracerLines[plr] then
        pcall(function() tracerLines[plr]:Remove() end)
        tracerLines[plr] = nil
    end
end
...
RunService:BindToRenderStep(TRACER_BIND_NAME, 202, function()
    ...
    local myC = LocalPlayer.Character
    if not myC then cleanAllTracers(); return end

    local vp = Camera.ViewportSize
    local origin = Vector2.new(vp.X / 2, 0) -- top-center of screen

    local seen = {}

    for _, plr in ipairs(Players:GetPlayers()) do
        if plr == LocalPlayer then continue end
        if Config.EnemyOnly and LocalPlayer.Team and plr.Team == LocalPlayer.Team then continue end

        local ch = plr.Character
        if not ch or not ch.Parent then cleanTracer(plr); continue end

        local head = ch:FindFirstChild("Head")
        local hum = ch:FindFirstChildOfClass("Humanoid")
        if not head or not hum or hum.Health <= 0 then cleanTracer(plr); continue end

        -- Top of head = head center + half head height (Head size Y is typically 1.2, so +0.6)
        local headTop = head.Position + Vector3.new(0, head.Size.Y / 2, 0)
        local screenPos, onScreen = Camera:WorldToViewportPoint(headTop)
        if not onScreen then cleanTracer(plr); continue end

        seen[plr] = true

        -- Create Drawing line if needed
    ...
    -- Hide/remove lines for players no longer tracked
...
ClsBtn.MouseButton1Click:Connect(function()
    ...
    stopFly()
    if aimlockCircle then aimlockCircle:Destroy() end
    if aimlockDot then aimlockDot:Destroy() end
    pcall(function() RunService:UnbindFromRenderStep(AIMLOCK_BIND_NAME) end)
    pcall(function() RunService:UnbindFromRenderStep(TRACER_BIND_NAME) end)
    cleanAllTracers()
    ...
    -- Clear globals
    _G.AUREN_ESP = nil; _G.AUREN_RENDER = nil
